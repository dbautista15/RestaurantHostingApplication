  ...
  try {
    ...
    const enrichedTables = tables.map(table => {
      ...
      return {
        ...
        timeInCurrentState: table.assignedAt ? 
          Math.floor((Date.now() - new Date(table.assignedAt)) / (1000 * 60)) : 0,
    ...
    const statistics = {
      tables: {
        ...
        available: enrichedTables.filter(t => t.isActive && t.state === 'available').length,
        assigned: enrichedTables.filter(t => t.isActive && t.state === 'assigned').length,
        occupied: enrichedTables.filter(t => t.isActive && t.state === 'occupied').length
      },
      waitlist: {
        total: waitlist.length,
        avgWaitTime: waitlist.length > 0 
          ? Math.round(waitlist.reduce((sum, party) => {
              return sum + Math.floor((Date.now() - new Date(party.createdAt)) / (1000 * 60));
            }, 0) / waitlist.length)
          : 0,
        byPriority: {
          normal: waitlist.filter(p => p.priority === 'normal').length,
          large_party: waitlist.filter(p => p.priority === 'large_party').length,
          coworker: waitlist.filter(p => p.priority === 'coworker').length
        }
      },
      capacity: {
        totalSeats: enrichedTables.reduce((sum, t) => sum + (t.isActive ? t.capacity : 0), 0),
        occupiedSeats: enrichedTables.reduce((sum, t) => 
          sum + (t.isActive && t.state === 'occupied' ? (t.partySize || 0) : 0), 0
        ),
    ...
    if (statistics.capacity.totalSeats > 0) {
      statistics.capacity.utilizationPercent = Math.round(
        (statistics.capacity.occupiedSeats / statistics.capacity.totalSeats) * 100
      );
    }
    ...
    res.json({
      ...
      data: {
        ...
        waitlist: waitlist.map(entry => ({
          ...
          actualWaitTime: Math.floor((Date.now() - new Date(entry.createdAt)) / (1000 * 60)),

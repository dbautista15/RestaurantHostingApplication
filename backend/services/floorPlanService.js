...
const {
  RESTAURANT_TABLES,
  getTableByNumber,
} = require("../config/restaurantLayout");
...
class FloorPlanService {
  ...
  async getFloorPlanData(requestingUser) {
    try {
      ...
      const tables = await Table.find({}).populate(
        "assignedWaiter",
        "userName role section clockInNumber"
      );
      ...
      const stats = {
        available: floorPlanTables.filter(
          (t) => t.isActive && t.state === "available"
        ).length,
        assigned: floorPlanTables.filter(
          (t) => t.isActive && t.state === "assigned"
        ).length,
        occupied: floorPlanTables.filter(
  ...
  getTableStyleHint(table) {
    ...
    if (table.state === "available")
      hints.push("hover:ring-2 hover:ring-green-400");
    if (table.state === "occupied")
      hints.push("hover:ring-2 hover:ring-red-400");
  ...
  getWaiterColor(section) {
    const colors = [
      { bg: "#ef4444", border: "#ef4444" }, // red
      { bg: "#22c55e", border: "#22c55e" }, // green
      { bg: "#f59e0b", border: "#f59e0b" }, // yellow
      { bg: "#3b82f6", border: "#3b82f6" }, // blue
      { bg: "#a855f7", border: "#a855f7" }, // purple
      { bg: "#ec4899", border: "#ec4899" }, // pink
      { bg: "#14b8a6", border: "#14b8a6" }, // teal
    ];
  ...
  async handleTableDrop(tableId, newPosition, user) {
    if (user.role !== "host") {
      throw new Error("Only hosts can move tables");
    }
